...
class WeightTrackDatabaseTest {
    ...
    private val PROFILE = WeightTrackDatabase.DEFAULT_PROFILE_ID
    ...
    fun `an inserted reading comes back`() = runTest {
        ...
        val all = weightDao.observeAll(PROFILE).first()
    ...
    fun `readings are listed newest first`() = runTest {
        ...
        assertThat(weightDao.observeAll(PROFILE).first().map { it.grams })
    ...
    fun `daily averages collapse several readings on one day`() = runTest {
        ...
        val rows = weightDao.observeDailyAverages(PROFILE).first()
    ...
    fun `latest and earliest bracket the log`() = runTest {
        ...
        assertThat(weightDao.latest(PROFILE)!!.grams).isEqualTo(78_000)
        assertThat(weightDao.earliest(PROFILE)!!.grams).isEqualTo(80_000)
    }
    ...
    fun `a repeated sync of the same record does not duplicate it`() = runTest {
        ...
        assertThat(weightDao.count(PROFILE)).isEqualTo(1)
        assertThat(firstId).isEqualTo(secondId)
        assertThat(secondId).isEqualTo(thirdId)
        assertThat(weightDao.latest(PROFILE)!!.grams).isEqualTo(80_200)
    }
    ...
    fun `a record recognised only by its health connect id still updates in place`() = runTest {
        ...
        assertThat(weightDao.count(PROFILE)).isEqualTo(1)
        assertThat(weightDao.latest(PROFILE)!!.grams).isEqualTo(80_500)
    }
    ...
    fun `distinct records still both land`() = runTest {
        ...
        assertThat(weightDao.count(PROFILE)).isEqualTo(2)
    }
    ...
    fun `search matches notes and tags`() = runTest {
        ...
        assertThat(weightDao.search(PROFILE, "flight").first()).hasSize(1)
        assertThat(weightDao.search(PROFILE, "TRAVEL").first()).hasSize(1)
        assertThat(weightDao.search(PROFILE, "").first()).hasSize(2)
        assertThat(weightDao.search(PROFILE, "nothing here").first()).isEmpty()
    }
    ...
    fun `bulk delete removes exactly the chosen rows`() = runTest {
        ...
        assertThat(weightDao.count(PROFILE)).isEqualTo(1)
        assertThat(weightDao.latest(PROFILE)!!.grams).isEqualTo(79_000)
    }
    ...
    fun `body fat lookup finds the newest reading that carries one`() = runTest {
        ...
        weightDao.insert(entry(day = 5, grams = 79_000))
        assertThat(weightDao.latestWithBodyFat(PROFILE)!!.bodyFatPercent).isWithin(1e-9).of(22.0)
    }
    ...
    fun `latest measurement per type ignores older readings`() = runTest {
        ...
        val latest = measurementDao.latestPerType(PROFILE).associateBy { it.type }
    ...
    fun `setting a goal retires the previous one`() = runTest {
        ...
        assertThat(goalDao.active(PROFILE)!!.targetGrams).isEqualTo(85_000)
        assertThat(goalDao.observeAll(PROFILE).first()).hasSize(2)
        assertThat(goalDao.observeAll(PROFILE).first().count { it.active }).isEqualTo(1)
    }

    @Test
    fun `no goal means no active goal`() = runTest {
        assertThat(goalDao.active(PROFILE)).isNull()
        assertThat(goalDao.observeActive(PROFILE).first()).isNull()
    }
}
